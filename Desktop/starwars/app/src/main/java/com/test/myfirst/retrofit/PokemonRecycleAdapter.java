...
public class PokemonRecycleAdapter extends RecyclerView.Adapter<PokemonRecycleAdapter.Viewholder> {
    private ArrayList<Result> starWarArrayList;
    private Context c;
    private View.OnClickListener pokemonClickListener;

    public PokemonRecycleAdapter(ArrayList<Result> starWarArrayList, Context c) {
        this.starWarArrayList = starWarArrayList;
        this.c = c;
    }
    ...
    public void onBindViewHolder(@NonNull Viewholder holder, int position) {
      //  Glide.with(c).asBitmap().load(starWarArrayList.get(position).getImage()).into(holder.pokemonImage);
        holder.pokemonName.setText(starWarArrayList.get(position).getName());
    }
